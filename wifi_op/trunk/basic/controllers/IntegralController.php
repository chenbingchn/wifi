...
//    }
    ...
    public function actionIndex() {
        ...
        if (isset($param['integral_id']) && !empty($param['integral_id'])) {
            $dataProvider->query->andwhere("id = {$param['integral_id']}");
        }
        if (isset($param['integral_name']) && !empty($param['integral_name'])) {
            $dataProvider->query->andwhere(['like', 'Title', $param['integral_name']]);
        }
        if (isset($param['integral_cate']) && !empty($param['integral_cate'])) {
            $dataProvider->query->andwhere("cat_id = {$param['integral_cate']}");
        }
        if (isset($param['integral_state']) && !empty($param['integral_state'])) {
            if($param['integral_state'] ==1){
                $dataProvider->query->andwhere("total = 0");
                $dataProvider->query->orWhere("`usage` >= `ration`");
            }elseif ($param['integral_state'] ==2){
                 $dataProvider->query->andwhere( "'".date('Y-m-d')  ."' < start_time or end_time < '" .date('Y-m-d')."'");
            }elseif ($param['integral_state'] ==3) {
                $dataProvider->query->andwhere( "'".date('Y-m-d')  ."' >= start_time and end_time >='" .date('Y-m-d')."'");
                $dataProvider->query->andwhere("total != 0");
                $dataProvider->query->andWhere("`usage` < `ration`");
            }
        }
        if(isset($param['start_time']) && !empty($param['start_time'])){
            $dataProvider->query->andWhere("start_time >= '".$param['start_time']."'");
        }
        if(isset($param['end_time']) && !empty($param['end_time'])){
            $dataProvider->query->andWhere("end_time <= '".$param['end_time']."'");
        }
